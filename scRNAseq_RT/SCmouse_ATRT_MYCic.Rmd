...
for(c in levels(old.clust)){
  ...
  tiff(paste0("atrt_MYCic_oldClust", c, "_cells.tiff"), width = 900, height = 700)
...
can.testis <- gene.tab$`Family member`
...
tiff("./cancer_testis/MYC_IC_cancer_testis_score.tiff", width = 900, height = 700)
...
write.table(tum_mat, file = "/marta_home/scRNAseq_Ranalysis/Seurat_v3/COO-cibersort/MYC_IC_bulk.txt",
...
write.table(tum_mat, file = "/marta_home/scRNAseq_Ranalysis/Seurat_v3/COO-cibersort/MYC_IC_bulk_clust.txt",
            quote = FALSE, sep = "\t", row.names = F, col.names = T)
...
```{r}
saveRDS(atrt.integrated, file = "/marta_home/GEO_COO/single_cell/MYC_IC_integrated_seurat_obj.rds")
```